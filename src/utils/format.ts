...
export function getTimeRemainingColor(dateStr: string | undefined): string {
    ...
    if (diffMs <= 0) return 'success'; // Already-expired also counts as success (about to reset or already reset)

    const diffHrs = diffMs / (1000 * 60 * 60);

    if (diffHrs < 1) return 'success';   // < 1h: green (resetting soon)
    if (diffHrs < 6) return 'warning';   // 1-6h: amber (waiting)
    return 'neutral';                   // > 6h: gray (long wait)
}
